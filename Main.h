...
class Curtain : public ShootBehavior {
protected:
	float start;
	float end;
public:
	Curtain(float firing_rate, const sf::Color& bullet_color, float start, float end) : ShootBehavior(firing_rate, bullet_color), start(start), end(end) {}
	void behave(sf::CircleShape& form, float dt) override;
};
...
public:
	Enemy(const sf::Vector2f& start_pos, ShootBehavior* shoot_behavior, RotateBehavior* rotate_behavior) : Entity(start_pos, Radii::ENEMY, Colors::ENEMYC), shoot_behavior(shoot_behavior), rotate_behavior(rotate_behavior) {
